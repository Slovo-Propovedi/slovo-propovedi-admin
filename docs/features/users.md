# Фронтенд: пользователи (users)

Фронтовая часть домена users: список с клиентским поиском, деталь (смена пароля и удаление), создание/редактирование, форма. Данные пользователей приходят с backend API.
