...
- **Поля:**
  - «Основное»: название, исполнитель, книга; «Глава (с)» / «Глава (по)» (`number`, для диапазона глав, парсятся через `parseChapter` → `number | [n, n] | null`); «Стихи» (`text`, одиночный стих, диапазон или разрозненные отрезки через запятую, парсятся через `parseVerseInput` → `number | [n, n] | VerseSegment[] | null`); описание.
