...
- **Что показывается:** заголовок «Проповеди», подзаголовок «Все аудиозаписи и тексты, доступные на сайте», кнопка «Загрузить проповедь». Поле `<Input>` «Поиск» (placeholder «Название, проповедник, книга…»). Плоский список `.list-grid` карточек `.list-item`: обложка/плейсхолдер, название, подзаголовок «Проповедник · Книга глава:стихи» (`formatReference`; нотация поддерживает диапазоны глав и стихов — `3:16–18`, `3:16–4:2`, `3–4` — и разрозненные отрезки стихов — `1:9–18, 20`), бейджи по наличию медиа: `аудио` (золотой), `youtube`, `текст` (нейтральные). Под списком — `Pagination`.
- **Откуда данные:** `sermonControllerFindAllOptions({ query: { search: debouncedTerm || undefined, page, limit: 20 } })` (`createQuery`) — **оффсетная пагинация**: страница `page` по `limit` = 20 записей. `take`/`cursor` на этом экране **не шлются** — backend отклоняет их вместе с `page`/`limit` (взаимоисключение). `placeholderData: keepPreviousData` — при перелистывании предыдущая страница остаётся видимой, пока грузится следующая (без мигания).
