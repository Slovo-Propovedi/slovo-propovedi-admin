...
- **Что показывается:** заголовок «Проповеди», подзаголовок «Все аудиозаписи и тексты, доступные на сайте», кнопка «Загрузить проповедь». Тулубар `.list-filters`: поле `<Input>` «Поиск» (placeholder «Название, проповедник, книга…»), `<Select>` «Сортировка» («По дате»/«По названию»/«По автору»/«По плейлисту») и `<Select>` «Направление» («По возрастанию»/«По убыванию»). Плоский список `.list-grid` карточек `.list-item`: обложка/плейсхолдер, название, подзаголовок «Проповедник · Книга глава:стихи» (`formatReference`; нотация поддерживает диапазоны глав и стихов — `3:16–18`, `3:16–4:2`, `3–4` — и разрозненные отрезки стихов — `1:9–18, 20`), бейджи по наличию медиа: `аудио` (золотой), `youtube`, `текст` (нейтральные). Под списком — `Pagination`.
- **Откуда данные:** `sermonControllerFindAllOptions({ query: { search: debouncedTerm || undefined, page, limit: 20, sort, order } })` (`createQuery`) — **оффсетная пагинация**: страница `page` по `limit` = 20 записей. `take`/`cursor` на этом экране **не шлются** — backend отклоняет их вместе с `page`/`limit` (взаимоисключение). Серверная сортировка: `sort` ∈ {`date`,`title`,`artist`,`playlist`}, `order` ∈ {`asc`,`desc`}; дефолт — `date`/`desc`. Смена `sort` сбрасывает `page` на 1 и **подтягивает направление к документированному дефолту** (`desc` для `date`, иначе `asc`); смена `order` тоже сбрасывает страницу. При активном `search` backend игнорирует `sort`/`order` (побеждает релевантность), но они остаются в запросе, чтобы вернуться при очистке поиска. `placeholderData: keepPreviousData` — при перелистывании предыдущая страница остаётся видимой, пока грузится следующая (без мигания).
- **Debounced поиск:** `<Input>` «Поиск» + `debounce(300)` → реактивный `debouncedTerm`. Пустой термин **не шлёт `search`** (ключ `undefined`), поэтому первичная загрузка — полная выборка. Ввод с паузой 300 мс перезапрашивает список и **сбрасывает страницу на 1**.
- **Пагинация:** `pageCount = ceil(count / 20)`; `Pagination` рендерится при `pageCount > 1`; переход по странице меняет `page` (query-key включает `page`/`limit`/`search`/`sort`/`order`).
- **Компоненты:** `Button`, `EmptyState`, `Icon`, `Input`, `LoadingSpinner`, `Pagination`, `Select`.
...
- **Поля:**
  - «Основное»: название, исполнитель, книга; «Глава (с)» / «Глава (по)» (`number`, для диапазона глав, парсятся через `parseChapter` → `number | [n, n] | null`); «Стихи» — **условное поле**: при пустом «Глава (по)» текстовый инпут (одиночный стих, диапазон или разрозненные отрезки через запятую, парсятся через `parseVerseInput` → `number | [n, n] | VerseSegment[] | null`), при заполненном «Глава (по)» — пара number-инпутов «Стих (с)» / «Стих (по)» (`min="1" step="1"`; оба заполнены → `[start, end]`, оба пусты → `null`, заполнен один → сабмит заблокирован); описание.
  ...
  - В режимах **create** и **edit** — блок «Плейлисты»: **поисковый** `CheckboxList` (`playlistControllerFindAllOptions({ query: { search: debouncedTerm || undefined, sort: 'title', order: 'asc' } })` с `debounce(300)`; `selectedPlaylistIds` — источник истины и переживает поиск; рядом с поиском счётчик «Выбрано: N», когда выборка непуста). Список пикера обёрнут в `.checkbox-list-scroll` (`max-height: 1000px; overflow-y: auto`) — длинный каталог не растягивает форму. Состояния блока: загрузка — `LoadingSpinner`, ошибка — «Не удалось загрузить плейлисты», активный поиск без совпадений — «Ничего не найдено».
...
- **Валидация:** без клиентского zod — HTML `required`/`min` + backend `strictObject`. Nullable-поля (`description`, `book`, `chapter`, `verse`, `youtubeUrl`, `audioUrl`, `textFileUrl`) шлют `null` при очистке против `undefined` («не трогать»). Поле «Стихи» дополнительно валидируется **живо** (derived-состояние): непустой непарсящийся текст («9, abc», «9–», «0») или наполовину заполненная пара «Стих (с)/(по)» блокируют сабмит с инлайн-ошибкой — невалидный ввод никогда не превращается в `null` (это молча стёрло бы сохранённые стихи).
