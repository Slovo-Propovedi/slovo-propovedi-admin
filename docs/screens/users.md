...
- **Что показывается:** заголовок «Пользователи», подзаголовок «Управление администраторами системы.», кнопка «Создать» (+ значок `plus`). Поле `<Input>` «Поиск» (placeholder «Имя, email или логин…», hint «Фильтрует только загруженную страницу.»). Список `.list-grid` карточек `.list-item`: обложка-плейсхолдер (первая буква имени), имя, подзаголовок — email, бейдж `badge-gold` — роль (`ROLE_LABELS[user.role]`), бейдж `badge-neutral` — username. Под списком — `Pagination`.
- **Откуда данные:** `usersControllerFindAllOptions({ query: { page, limit: 20 } })` (`createQuery`) — **оффсетная пагинация**; ответ — обёртка `{ users, count }` (`AllUsersResponse`), а не плоский массив. `placeholderData: keepPreviousData` — при перелистывании предыдущая страница остаётся видимой, пока грузится следующая (без мигания).
- **Клиентский поиск + debounce:** `<Input>` «Поиск» + `debounce(300)` → реактивный `debouncedTerm`. Фильтрация — **на клиенте** по `name`/`email`/`username` (у эндпоинта **нет** серверного `search`). Фильтр работает **только по загруженной странице** (об этом говорит hint под инпутом); пагинация переключает полные страницы. Пустой термин показывает всех пользователей текущей страницы.
- **Пагинация:** `pageCount = ceil(count / 20)`; `Pagination` рендерится при `pageCount > 1`; переход по странице меняет `page` (query-key включает `page`/`limit`).
...
- **Состояния:**
  ...
  - фильтр без совпадений на странице (`debouncedTerm !== ''` и отфильтрованный список пуст): `EmptyState` «Ничего не найдено» с подсказкой «По запросу «{debouncedTerm}» на этой странице ничего не найдено»;
