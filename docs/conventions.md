...
- **Структура каждого типа документа** предсказуема и задана в [`README.md`](./README.md) (карта) — новый документ создаётся по тому же шаблону, что и соседние. Каждый документ заканчивается разделом `## Связанные документы`.
...
- **Решения не стираются:** отменённое решение помечается зачёркиванием с объяснением в `decisions.md` (раздел Superseded), а не удаляется.
