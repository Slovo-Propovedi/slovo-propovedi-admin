...
Этот документ объясняет устройство admin-панели «Слово.Проповеди» — **Svelte 5 SPA** на Vite. Код — истина; здесь фиксируются мотивация и раскладка, а не дублирование правил линтеров/тайпчекеров. За командами и OpenAPI-first workflow — [`conventions.md`](./conventions.md).
...
- `src/lib/router/Router.svelte` — декларативная таблица маршрутов, `$derived`-матчинг, 404→`/`, guard-обёртка; **admin-only guard для `/users*`** (страница не монтируется для не-admin, редирект на `/`).
